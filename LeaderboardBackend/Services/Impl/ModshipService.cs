...
public class ModshipService : IModshipService
{
    ...
    {
        return await _applicationContext.Modships.FirstOrDefaultAsync(
            modship => modship.UserId == userId
        );
    }

    public async Task<Modship?> GetModshipForLeaderboard(long leaderboardId, Guid userId)
    {
        return await _applicationContext.Modships.SingleOrDefaultAsync(
            modship => modship.LeaderboardId == leaderboardId && modship.UserId == userId
        );
    }
